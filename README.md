...
`glob` matches the complete string, ignores case, and treats only `*` as a wildcard. An array of glob patterns matches any member. A Bash `command` glob is tested against both the exact command and a normalized matching view. That view trims surrounding whitespace and ignores one leading alias-escape backslash. A directly invoked `rm` path is represented as `rm` when its arguments include both recursive and force flags. The Host does not change the executed input: Approval displays and binds the exact Host-final command. A Host transform requires fresh Approval if that input changes.
